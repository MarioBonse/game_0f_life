...
// in this file we use a reduce so the primes should be sorted
...
int main(int argc, char *argv[]) {    
    if (argc<4) {
        std::cerr << "use: " << argv[0]  << " number1 number2 nw [print=off|on]\n";
        return -1;
    }
    ...
    // setup a parallel for
    ParallelForReduce<std::vector<ull>>* pf;
    pf = new ParallelForReduce<std::vector<ull>>(nw);

    std::vector<ull> results;
    results.reserve((size_t)(n2-n1)/log(n1));
    ull prime;

    pf->parallel_reduce(results, {}, n1, n2,    // start, stop indexes
                            1,          // step size
                            0,          // chunk size (0=static, >0=dynamic) maybe dimanic is better for this task?
                            [&](ull prime, std::vector<ull> &results)  {
                            if(is_prime(prime)) {
                                //lock mutex
                                results.push_back(prime);
                                }
                            },
                            [](std::vector<ull> &results, const std::vector<ull>& elem) {results.insert(results.end(), elem.begin(), elem.end()); }
                            );
